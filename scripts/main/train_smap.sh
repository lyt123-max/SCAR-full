...
bundle = load_raw_dataset_bundle("SMAP", os.environ["DATA_ROOT"])
...
print_header() {
  local stage_name="$1"
  local log_file="$2"
  echo "[SMAP Script] repo_root=${REPO_ROOT}"
  echo "[SMAP Script] stage=${stage_name} exp_name=${EXP_NAME} device=${DEVICE}"
  echo "[SMAP Script] data_root=${DATA_ROOT} artifact_root=${ARTIFACT_ROOT}"
  echo "[SMAP Script] resume=${RESUME} log_file=${log_file}"
  echo "[SMAP Script] conda_prefix=${CONDA_PREFIX:-<unset>}"
  echo "[SMAP Script] use_faiss=${USE_FAISS} faiss_use_gpu=${FAISS_USE_GPU}"
  echo "[SMAP Script] mask_ratio=${MASK_RATIO} n_mask_groups=${N_MASK_GROUPS}"
  echo "[SMAP Script] d_z=${D_Z} patch_sizes=${PATCH_SIZES}"
  echo "[SMAP Script] val_split_mode=${VAL_SPLIT_MODE}"
  echo "[SMAP Script] evaluation_score_key=${EVALUATION_SCORE_KEY}"
  echo "[SMAP Script] train_batch_size=${TRAIN_BATCH_SIZE} val_batch_size=${VAL_BATCH_SIZE}"
  echo "[SMAP Script] memory_batch_size=${MEMORY_BATCH_SIZE} test_batch_size=${TEST_BATCH_SIZE}"
  echo "[SMAP Script] detected_n_channels=${CHANNELS}"
}
...
if [[ "${STAGE}" == "stage_c" ]]; then
  echo "[SMAP Script] stage_c has been removed. Use stage_a, stage_b, test, or full."
  exit 1
fi

if [[ "${STAGE}" == "full" ]]; then
  echo "[SMAP Script] full mode logs:"
  echo "[SMAP Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_a.log"
  echo "[SMAP Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_b.log"
  echo "[SMAP Script]   ${RUN_LOG_DIR}/${EXP_NAME}_test.log"
