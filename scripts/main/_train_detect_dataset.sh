...
if [[ "$#" -lt 2 ]]; then
  echo "Usage: $0 <dataset_name> <default_exp_base_name> [stage] [exp_base_name]" >&2
  exit 1
fi

DATASET_NAME="$1"
DEFAULT_EXP_BASE_NAME="$2"
STAGE="${3:-full}"
EXP_BASE_NAME="${4:-${DEFAULT_EXP_BASE_NAME}}"
...
COMPLETION_DROPOUT="${COMPLETION_DROPOUT:-0.1}"
...
mkdir -p "${RUN_LOG_DIR}"
CHANNELS="$(DATA_ROOT="${DATA_ROOT}" DATASET_NAME="${DATASET_NAME}" python - <<'PY'
import os
from coremad.data import load_raw_dataset_bundle

bundle = load_raw_dataset_bundle(os.environ["DATASET_NAME"], os.environ["DATA_ROOT"])
...
COMMON_ARGS+=(--evaluation_score_key "${EVALUATION_SCORE_KEY}")
...
print_header() {
  local stage_name="$1"
  local log_file="$2"
  echo "[${DATASET_TAG} Script] repo_root=${REPO_ROOT}"
  echo "[${DATASET_TAG} Script] stage=${stage_name} exp_name=${EXP_NAME} device=${DEVICE}"
  echo "[${DATASET_TAG} Script] data_root=${DATA_ROOT} artifact_root=${ARTIFACT_ROOT}"
  echo "[${DATASET_TAG} Script] resume=${RESUME} log_file=${log_file}"
  echo "[${DATASET_TAG} Script] conda_prefix=${CONDA_PREFIX:-<unset>}"
  echo "[${DATASET_TAG} Script] use_faiss=${USE_FAISS} faiss_use_gpu=${FAISS_USE_GPU}"
  echo "[${DATASET_TAG} Script] mask_ratio=${MASK_RATIO} n_mask_groups=${N_MASK_GROUPS}"
  echo "[${DATASET_TAG} Script] completion_dropout=${COMPLETION_DROPOUT}"
  echo "[${DATASET_TAG} Script] d_z=${D_Z} patch_sizes=${PATCH_SIZES}"
  echo "[${DATASET_TAG} Script] val_split_mode=${VAL_SPLIT_MODE}"
  echo "[${DATASET_TAG} Script] evaluation_score_key=${EVALUATION_SCORE_KEY}"
  echo "[${DATASET_TAG} Script] max_train_windows=${MAX_TRAIN_WINDOWS} max_test_windows=${MAX_TEST_WINDOWS}"
  echo "[${DATASET_TAG} Script] train_batch_size=${TRAIN_BATCH_SIZE} val_batch_size=${VAL_BATCH_SIZE}"
  echo "[${DATASET_TAG} Script] memory_batch_size=${MEMORY_BATCH_SIZE} test_batch_size=${TEST_BATCH_SIZE}"
  echo "[${DATASET_TAG} Script] detected_n_channels=${CHANNELS}"
}
...
if [[ "${STAGE}" == "stage_c" ]]; then
  echo "[${DATASET_TAG} Script] stage_c has been removed. Use stage_a, stage_b, test, or full."
  exit 1
fi

if [[ "${STAGE}" == "full" ]]; then
  echo "[${DATASET_TAG} Script] full mode logs:"
  echo "[${DATASET_TAG} Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_a.log"
  echo "[${DATASET_TAG} Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_b.log"
  echo "[${DATASET_TAG} Script]   ${RUN_LOG_DIR}/${EXP_NAME}_test.log"
