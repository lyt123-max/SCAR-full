...
# SMD is large enough that stride=1 can blow up host RAM during memory-bank build
# because Stage B materializes all patch embeddings before coreset reduction.
MEMORY_BUILD_STRIDE="${MEMORY_BUILD_STRIDE:-4}"

VAL_RATIO="${VAL_RATIO:-0.15}"
VAL_GAP="${VAL_GAP:-1}"
VAL_MIN_TRAIN_WINDOWS="${VAL_MIN_TRAIN_WINDOWS:-50}"
# Aggregated SMD is less stable under a plain tail split because the validation
# tail can overrepresent the last machine chunk. Interleaving gives a fairer
# Stage-A validation signal.
...
bundle = load_raw_dataset_bundle("SMD", os.environ["DATA_ROOT"])
...
print_header() {
  local stage_name="$1"
  local log_file="$2"
  echo "[SMD Script] repo_root=${REPO_ROOT}"
  echo "[SMD Script] stage=${stage_name} exp_name=${EXP_NAME} device=${DEVICE}"
  echo "[SMD Script] data_root=${DATA_ROOT} artifact_root=${ARTIFACT_ROOT}"
  echo "[SMD Script] resume=${RESUME} log_file=${log_file}"
  echo "[SMD Script] conda_prefix=${CONDA_PREFIX:-<unset>}"
  echo "[SMD Script] use_faiss=${USE_FAISS} faiss_use_gpu=${FAISS_USE_GPU}"
  echo "[SMD Script] mask_ratio=${MASK_RATIO} n_mask_groups=${N_MASK_GROUPS}"
  echo "[SMD Script] d_z=${D_Z} patch_sizes=${PATCH_SIZES}"
  echo "[SMD Script] val_split_mode=${VAL_SPLIT_MODE}"
  echo "[SMD Script] evaluation_score_key=${EVALUATION_SCORE_KEY}"
  echo "[SMD Script] train_batch_size=${TRAIN_BATCH_SIZE} val_batch_size=${VAL_BATCH_SIZE}"
  echo "[SMD Script] memory_batch_size=${MEMORY_BATCH_SIZE} test_batch_size=${TEST_BATCH_SIZE}"
  echo "[SMD Script] detected_n_channels=${CHANNELS}"
}
...
if [[ "${STAGE}" == "stage_c" ]]; then
  echo "[SMD Script] stage_c has been removed. Use stage_a, stage_b, test, or full."
  exit 1
fi

if [[ "${STAGE}" == "full" ]]; then
  echo "[SMD Script] full mode logs:"
  echo "[SMD Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_a.log"
  echo "[SMD Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_b.log"
  echo "[SMD Script]   ${RUN_LOG_DIR}/${EXP_NAME}_test.log"
