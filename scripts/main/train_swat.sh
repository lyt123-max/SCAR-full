...
# SWaT has very dense and highly redundant windows; use a looser memory-build stride
# so stage_b remains tractable while keeping test-time stride unchanged.
MEMORY_BUILD_STRIDE="${MEMORY_BUILD_STRIDE:-2}"
...
COMPLETION_DROPOUT="${COMPLETION_DROPOUT:-0.2}"
...
bundle = load_raw_dataset_bundle("SWAT", os.environ["DATA_ROOT"])
...
print_header() {
  local stage_name="$1"
  local log_file="$2"
  echo "[SWAT Script] repo_root=${REPO_ROOT}"
  echo "[SWAT Script] stage=${stage_name} exp_name=${EXP_NAME} device=${DEVICE}"
  echo "[SWAT Script] data_root=${DATA_ROOT} artifact_root=${ARTIFACT_ROOT}"
  echo "[SWAT Script] resume=${RESUME} log_file=${log_file}"
  echo "[SWAT Script] conda_prefix=${CONDA_PREFIX:-<unset>}"
  echo "[SWAT Script] use_faiss=${USE_FAISS} faiss_use_gpu=${FAISS_USE_GPU}"
  echo "[SWAT Script] mask_ratio=${MASK_RATIO} n_mask_groups=${N_MASK_GROUPS}"
  echo "[SWAT Script] completion_dropout=${COMPLETION_DROPOUT}"
  echo "[SWAT Script] d_z=${D_Z} patch_sizes=${PATCH_SIZES}"
  echo "[SWAT Script] val_split_mode=${VAL_SPLIT_MODE}"
  echo "[SWAT Script] memory_build_stride=${MEMORY_BUILD_STRIDE} coreset_max_patches_per_scale=${CORESET_MAX_PATCHES_PER_SCALE}"
  echo "[SWAT Script] evaluation_score_key=${EVALUATION_SCORE_KEY}"
  echo "[SWAT Script] train_batch_size=${TRAIN_BATCH_SIZE} val_batch_size=${VAL_BATCH_SIZE}"
  echo "[SWAT Script] memory_batch_size=${MEMORY_BATCH_SIZE} test_batch_size=${TEST_BATCH_SIZE}"
  echo "[SWAT Script] detected_n_channels=${CHANNELS}"
}
...
if [[ "${STAGE}" == "stage_c" ]]; then
  echo "[SWAT Script] stage_c has been removed. Use stage_a, stage_b, test, or full."
  exit 1
fi

if [[ "${STAGE}" == "full" ]]; then
  echo "[SWAT Script] full mode logs:"
  echo "[SWAT Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_a.log"
  echo "[SWAT Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_b.log"
  echo "[SWAT Script]   ${RUN_LOG_DIR}/${EXP_NAME}_test.log"
