...
bundle = load_raw_dataset_bundle("MSL", os.environ["DATA_ROOT"])
...
print_header() {
  local stage_name="$1"
  local log_file="$2"
  echo "[MSL Script] repo_root=${REPO_ROOT}"
  echo "[MSL Script] stage=${stage_name} exp_name=${EXP_NAME} device=${DEVICE}"
  echo "[MSL Script] data_root=${DATA_ROOT} artifact_root=${ARTIFACT_ROOT}"
  echo "[MSL Script] resume=${RESUME} log_file=${log_file}"
  echo "[MSL Script] conda_prefix=${CONDA_PREFIX:-<unset>}"
  echo "[MSL Script] use_faiss=${USE_FAISS} faiss_use_gpu=${FAISS_USE_GPU}"
  echo "[MSL Script] mask_ratio=${MASK_RATIO} n_mask_groups=${N_MASK_GROUPS}"
  echo "[MSL Script] d_z=${D_Z} patch_sizes=${PATCH_SIZES}"
  echo "[MSL Script] val_split_mode=${VAL_SPLIT_MODE}"
  echo "[MSL Script] evaluation_score_key=${EVALUATION_SCORE_KEY}"
  echo "[MSL Script] train_batch_size=${TRAIN_BATCH_SIZE} val_batch_size=${VAL_BATCH_SIZE}"
  echo "[MSL Script] memory_batch_size=${MEMORY_BATCH_SIZE} test_batch_size=${TEST_BATCH_SIZE}"
  echo "[MSL Script] detected_n_channels=${CHANNELS}"
}
...
if [[ "${STAGE}" == "stage_c" ]]; then
  echo "[MSL Script] stage_c has been removed. Use stage_a, stage_b, test, or full."
  exit 1
fi

if [[ "${STAGE}" == "full" ]]; then
  echo "[MSL Script] full mode logs:"
  echo "[MSL Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_a.log"
  echo "[MSL Script]   ${RUN_LOG_DIR}/${EXP_NAME}_stage_b.log"
  echo "[MSL Script]   ${RUN_LOG_DIR}/${EXP_NAME}_test.log"
