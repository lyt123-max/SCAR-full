...
ABLATIONS_STR="${ABLATIONS:-full wo_decomposition wo_state_aware_representation global_retrieval state_only_retrieval context_only_retrieval dual_condition_retrieval raw_max zscore_mean single_scale_short single_scale_long multi_scale}"
...
if [[ "${MATRIX_RESUME}" == "1" ]]; then
  ...
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/ablation_summary"
else
  DEFAULT_EXPERIMENT_TEMPLATE="{dataset_lower}_ablation_{ablation}_${RUN_TIMESTAMP}"
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/ablation_summary_${RUN_TIMESTAMP}"
fi
...
SUMMARY_SCORE_KEYS_STR="${SUMMARY_SCORE_KEYS:-}"
SUMMARY_METRIC_KEYS_STR="${SUMMARY_METRIC_KEYS:-}"
SUMMARY_MARKDOWN_METRIC_KEYS_STR="${SUMMARY_MARKDOWN_METRIC_KEYS:-}"
EXPERIMENT_NAME_TEMPLATE="${EXPERIMENT_NAME_TEMPLATE:-${DEFAULT_EXPERIMENT_TEMPLATE}}"

read -r -a DATASETS <<< "${DATASETS_STR}"
read -r -a ABLATIONS <<< "${ABLATIONS_STR}"
read -r -a SUMMARY_SCORE_KEYS <<< "${SUMMARY_SCORE_KEYS_STR}"
...
for dataset in "${DATASETS[@]}"; do
  for ablation in "${ABLATIONS[@]}"; do
    echo "[AblationMatrix] dataset=${dataset} ablation=${ablation} stage=${STAGE}"
...
if [[ "${COLLECT_RESULTS}" == "1" ]]; then
  ...
  if [[ ${#SUMMARY_SCORE_KEYS[@]} -gt 0 ]]; then
    SUMMARY_CMD+=(--score_keys "${SUMMARY_SCORE_KEYS[@]}")
  fi
  ...
  echo "[AblationMatrix] collecting summary results"
  "${SUMMARY_CMD[@]}"
fi
