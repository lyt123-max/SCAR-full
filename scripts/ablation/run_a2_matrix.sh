...
# Title: A2 ablation matrix (global, state-only, context-only, dual-condition)
...
ABLATIONS_STR="${ABLATIONS:-global_retrieval state_only_retrieval context_only_retrieval dual_condition_retrieval}"
...
if [[ "${MATRIX_RESUME}" == "1" ]]; then
  ...
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/a2_ablation_summary"
else
  DEFAULT_EXPERIMENT_TEMPLATE="{dataset_lower}_ablation_{ablation}_${RUN_TIMESTAMP}"
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/a2_ablation_summary_${RUN_TIMESTAMP}"
fi
...
for dataset in "${DATASETS[@]}"; do
  for ablation in "${ABLATIONS[@]}"; do
    echo "[A2Matrix] dataset=${dataset} ablation=${ablation} stage=${STAGE}"
...
if [[ "${COLLECT_RESULTS}" == "1" ]]; then
  ...
  echo "[A2Matrix] collecting summary results"
  "${SUMMARY_CMD[@]}"
fi
