...
# Title: A4 ablation matrix (temporal scales)
...
ABLATIONS_STR="${ABLATIONS:-single_scale_short single_scale_long multi_scale}"
...
if [[ "${MATRIX_RESUME}" == "1" ]]; then
  ...
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/a4_ablation_summary"
else
  DEFAULT_EXPERIMENT_TEMPLATE="{dataset_lower}_ablation_{ablation}_${RUN_TIMESTAMP}"
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/a4_ablation_summary_${RUN_TIMESTAMP}"
fi
...
for dataset in "${DATASETS[@]}"; do
  for ablation in "${ABLATIONS[@]}"; do
    echo "[A4Matrix] dataset=${dataset} ablation=${ablation} stage=${STAGE}"
...
if [[ "${COLLECT_RESULTS}" == "1" ]]; then
  ...
  echo "[A4Matrix] collecting summary results"
  "${SUMMARY_CMD[@]}"
fi
