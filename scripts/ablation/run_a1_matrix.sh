...
# Title: A1 ablation matrix (Full, w/o decomposition, w/o state-aware representation)
...
ABLATIONS_STR="${ABLATIONS:-full wo_decomposition wo_state_aware_representation}"
...
if [[ "${MATRIX_RESUME}" == "1" ]]; then
  ...
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/a1_ablation_summary"
else
  DEFAULT_EXPERIMENT_TEMPLATE="{dataset_lower}_ablation_{ablation}_${RUN_TIMESTAMP}"
  DEFAULT_SUMMARY_OUTPUT_PREFIX="${ARTIFACT_ROOT}/a1_ablation_summary_${RUN_TIMESTAMP}"
fi
...
for dataset in "${DATASETS[@]}"; do
  for ablation in "${ABLATIONS[@]}"; do
    echo "[A1Matrix] dataset=${dataset} ablation=${ablation} stage=${STAGE}"
...
if [[ "${COLLECT_RESULTS}" == "1" ]]; then
  ...
  echo "[A1Matrix] collecting summary results"
  "${SUMMARY_CMD[@]}"
fi
