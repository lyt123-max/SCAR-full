...
- 文件名 `_tr_N_` 中的 `N` 是官方半监督训练前缀边界；Stage A、Stage B、CDF 和 Z-score 拟合只能使用此前缀。官方清单中部分前缀仍含正标签，运行时保持完整 `tr_N` 前缀且不按标签过滤，并记录前缀异常数量与比例以供协议审计。文件名 `_1st_K_` 仅作为首异常元数据审计；若其与 CSV 标签不一致，记录声明值、观测值和一致性标记，但不得据此改变官方 `tr_N` 边界或拒绝序列。
