...
1. 所有方法使用相同的五个数据文件、训练/验证/测试边界、随机种子集合和 AUROC/AP
   evaluator。
2. 每个方法保留其官方模型结构、窗口长度、patch、损失函数和推荐训练超参数。
3. 每个“方法 × 数据集 × seed”运行生成完整 checkpoint、逐点分数、指标、资源记录、
   环境信息、日志和 SHA-256 清单。
...
- 聚合：三个 seed 分别保存，论文报告 mean 和 standard deviation。
...
和最终配置写入运行记录。不得逐 seed 或逐测试指标选择配置。
...
GPU 计时前后必须执行同步。推理先预热，再进行三次计时；正式值保存三次原始记录、
均值和标准差。训练时间直接来自三个正式 seed，不额外重复完整训练。
...
默认正式矩阵为 6 个 GPU 方法 × 5 个数据集 × 3 个 seed；实际不支持的组合保留状态
...
任何缺失 seed 都必须显示为 incomplete，不能用已有 seed 代替三 seed 平均。
...
可扩展性使用固定 seed `42`，与三 seed 主效率矩阵分开。
...
- incomplete/failed/unsupported 不进入正式均值。
...
1. 五个数据集均完成 SCAR 三 seed。
2. 每个可运行 GPU baseline 的支持组合均完成三 seed。
